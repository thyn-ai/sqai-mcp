...
This document describes how decisions are made for SQAI
(`thyn-ai/sqai-mcp`). It is intentionally lightweight and will evolve as
...
`thyn-ai/sqai` repository (see [CONTRIBUTING.md](./CONTRIBUTING.md)).
...
- The `@thyn-ai/sqai-mcp` package on npm (and the sibling `@thyn-ai/sqai`
  and `@thyn-ai/sqai-ai-sdk` packages) follows
